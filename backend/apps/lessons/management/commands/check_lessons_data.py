...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            if not table_exists:
                self.stdout.write(self.style.ERROR("❌ Table 'lessons_lesson' does NOT exist!"))
                return
            ...
            if count == 0:
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠️  Table 'lessons_lesson' exists but is EMPTY ({count} rows)"
                    )
                )
            else:
                self.stdout.write(self.style.SUCCESS(f"✅ Table 'lessons_lesson' has {count} rows"))
